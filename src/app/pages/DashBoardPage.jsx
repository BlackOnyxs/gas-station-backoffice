...
import { useAuthStore, useDashboardStore, useSocket } from '../../hooks';

import { format, startOfDay } from 'date-fns';
import { useSocketInvoice } from '../../hooks/';
...
const OilSvg = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 48 48"><mask id="IconifyId18b83327f97dd75770"><g fill="none" strokeLinejoin="round" strokeWidth="4"><path fill="#fff" stroke="#fff" d="M10 6h28v36H10z" /><path fill="#000" stroke="#000" strokeLinecap="round" d="M20.643 23.889c1.431-1.88 2.535-4.479 3.131-5.889c1.044 1.41 3.31 4.948 4.026 6.829c.894 2.35-1.342 5.171-4.026 5.171c-2.684 0-4.92-3.76-3.131-6.111Z" /><path stroke="#fff" strokeLinecap="round" d="M6 6h36M6 42h36M6 24h4m28 0h4" /></g></mask><path fill="#FFA500" d="M0 0h48v48H0z" mask="url(#IconifyId18b83327f97dd75770)" /></svg>
)
...
export const DashBoardPage = () => {
  ...
  const { statistics, isLoadingData, errorMessage, startLoadData } = useDashboardStore();
  ...
  useEffect(() => {
    // console.log(format(startOfDay(new Date()), "dd/MM/yyyy HH:mm:ss"), format(new Date(), "dd/MM/yyyy HH:mm:ss"))
    startLoadData( format(startOfDay(new Date()), "dd/MM/yyyy HH:mm:ss"), format(new Date(), "dd/MM/yyyy HH:mm:ss"))
    // return () => {
  ...
  const getFuelIcon = (fuelName) => {
    if ( fuelName ) {
      if ( fuelName.includes('Diesel') ) {
        return <Icon component={DieselIcon} />;
      } else if ( fuelName.includes('Gasolina 91') ) {
        return <Icon component={Gasoline91Icon} />;
      } else if ( fuelName.includes('Gasolina 95') ) {
        return <Icon component={Gasoline95Icon} />;
      }
    }
  }
  return (
    <div className="site-statistic-demo-card">
      <Row gutter={16} style={{ marginBottom: 50 }}>
        <Col span={6} style={{ marginBottom: 10 }} >
          <Card>
            <Statistic
              ...
              value={statistics.dailySellInvoices}
        ...
        {
          Array.isArray(statistics.fuelInventory) && statistics.fuelInventory.map(fuel => (
            (typeof fuel === 'object' && fuel !== null) && (
              <Col key={fuel._id} span={6} style={{ marginBottom: 10 }}>
                <Card>
                  <Statistic
                    style={{ textAlign: 'center', fontWeight: 'bold' }}
                    title={fuel.name}
                    value={fuel.inventory}
                    precision={2}
                    valueStyle={{ color: '#1E90FF' }}
                    prefix={getFuelIcon(fuel.name)}
                    suffix="L"
                  />
                </Card>
              </Col>
        ...
        <Col span={6}>
          <Card>
            <Statistic
              style={{ textAlign: 'center', fontWeight: 'bold' }}
              title="Productos"
              value={statistics.productsQuantity}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              style={{ textAlign: 'center', fontWeight: 'bold' }}
              title="Colaboradores"
              value={statistics.employeeQuantity}
        ...
        <Col span={6}>
          <Card>
            <Statistic
              style={{ textAlign: 'center', fontWeight: 'bold' }}
              title="Proovedores"
              value={statistics.providerQuantity}
